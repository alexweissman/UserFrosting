...
use UserFrosting\Sprinkle\Lms\Database\Models\Article;

class BlogController extends SimpleController

{

    public function pageBlogIndex(Request $request, Response $response, $args)
    {  
        $articles = Article::orderBy('created_at', 'desc')->get();
        return $this->ci->view->render($response, 'pages/blog/index.html.twig', [
            'articles' => $articles
        ]);
    }

    public function pageRules(Request $request, Response $response, $args)
    {  
        return $this->ci->view->render($response, 'pages/rules.html.twig');
    }

    public function pageBlogArticle(Request $request, Response $response, $args)
    {  
        $article = Article::where('slug', $args['blog_slug'])->first();
        if (!$article) {
            throw new NotFoundException();
        }
        return $this->ci->view->render($response, 'pages/blog/pageViewArticle.html.twig',[
            'article' => $article
        ]);
    }
